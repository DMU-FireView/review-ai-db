...
- `text/event-stream` 프레이밍을 직접 파싱해 `app.contracts.stream` 이벤트로 옮긴다.
- 스트림 오류를 이 서비스가 이미 쓰는 예외로 옮긴다.

수정 범위:
- [MODEL / EXTERNAL INTEGRATION]
- 크롤러 서버의 SSE 경로·프레이밍·오류 규약이 바뀌면 이 파일을 수정한다.
- base URL·timeout·재시도 상한은 `app.core.crawler_settings`에서 읽는다. 여기서 다시 정의하지 않는다.
- 이벤트 payload 계약은 `app.contracts.stream`에서, analyzer 점수 의미나 scoring 정책은
  이 파일에서 변경하지 않는다.
...
`app.core.crawler_settings`에는 이 값에 해당하는 항목이 없다. 스트림 수명과 달리 운영 중에
...
def stream_reviews_path(platform: str, product_id: str) -> str:
    ...
    return f"/{quote(platform, safe='')}/products/{quote(product_id, safe='')}/reviews/stream"
...
class CrawlerStreamClient:
    ...
    재연결 상한은 `app.core.crawler_settings`의 `max_retries`(`CRAWLER_MAX_RETRIES`)를 그대로 쓴다.
